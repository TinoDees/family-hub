...
- **Recipes & Meals** — recipe collection (from URLs, photos, even videos), cook mode,
  ...
  have ingredients.
- **Shopping lists** — shared lists, connected to the meal planner. Lists are **grouped
  by grocery category** (Fruit & Veg first, Cleaning last — a sensible store walk);
  every item is auto-categorised as it's typed or generated, with a quiet per-item
  selector to correct a wrong guess. **🧺 Staples** (Shopping → Staples) holds the
  things the household always needs beyond recipes — a bare name is a complete entry,
  and optional min/max/unit targets per staple power suggested quantities later. On any
  open list, **Add staples** pulls in every staple not already on it. See
  `docs/shopping-list-v2.md` for the roadmap (review worksheet, per-store aisle order,
  nearby specials).
