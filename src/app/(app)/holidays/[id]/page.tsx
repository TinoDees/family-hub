...
export default async function TripOverviewPage({
  ...
  const [{ data: trip }, { data: participants }, { data: expenses }, { data: album }] =
    await Promise.all([
      supabase
        .from("trips")
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="rounded-xl border border-stone-200 bg-white p-5">
        ...
        <p className="mt-3 text-xs text-stone-400">
          Guest logins for friends (they see only this trip) are coming next — for now you keep the record, they get the summary.
        </p>
      </div>
    </div>
  );
