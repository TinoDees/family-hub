  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f4;padding:24px 0;">
    <tr><td align="center">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;">
        <tr><td style="background:#1c1917;padding:20px 32px;color:#ffffff;font-size:18px;font-weight:bold;">🏠 Family Hub</td></tr>
        ...
        <tr><td style="padding:16px 32px;border-top:1px solid #e7e5e4;font-size:12px;color:#a8a29e;">
          Sent by Family Hub — your household OS.
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendInviteEmail({
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      <p>${inviterName} has invited you to join <strong>${householdName}</strong> on Family Hub as <strong>${role}</strong>.</p>
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: [to],
      subject: `Join ${householdName} on Family Hub`,
      html,
    }),
  });
